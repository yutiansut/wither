...
use fixtures::{Fixture, User, UserModelBadMigrations};
...
fn model_find_should_find_all_instances_of_model_with_no_filter_or_options() {
    ...
    let users_from_db: Vec<_> = User::find(db.clone(), None, None)
        .expect("Expected a successful lookup.")
        .into_iter()
        .collect();

    assert_eq!(users_from_db.len(), 1);
    assert!(users_from_db[0].is_ok());
    let userdb = users_from_db[0].unwrap();
...
fn model_find_should_find_instances_of_model_matching_filter() {
    ...
    let users_from_db: Vec<_> = User::find(db.clone(), Some(doc), None)
        .expect("Expected a successful lookup.")
        .into_iter()
        .collect();
